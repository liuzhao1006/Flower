...
import static com.lz.flower.camera.SimpleCamera.SELECTOR_CROP_BITMAP;
import static com.lz.flower.camera.SimpleCamera.SELECTOR_SCALED_BITMAP;
import static com.lz.flower.camera.SimpleCamera.SELECTOR_SRC_BITMAP;
...
class CameraView extends SurfaceView {
    ...
    void initCamera(SurfaceHolder holder, float pictureRate) {
        ...
        Log.i("刘朝","初始化相机展示方向是:" + displayOrientation);

        if (isNexus5X()) {
            displayOrientation = 270;
        }
        // TODO 相机方向调整,这个手机的相机方向需要设置info.orientation = 0;
        ...
        new AsyncTask<RawImage, Integer, Bitmap[]>() {
            ...
            protected Bitmap[] doInBackground(RawImage... params) {
                ...
                if (rotate && mCatcher != null) {
                    ...
                    Log.i("刘朝","屏幕的旋转角度是:"+mCatcher.getRotation() );
                    if (srcBitmap != bitmap) {
                        bitmap.recycle();
                    }
                } else {
    ...
    private static Bitmap scaleBitmap(@NonNull final Bitmap bitmap, int rotation) {
        ...
        switch (rotation) {
            case Surface.ROTATION_180:
                ...
                Log.i("刘朝:ROTATION_180->","x : " + x +"; y : " + y);
                break;
            case Surface.ROTATION_270:
                x = width - size - width / CameraMaskView.SIZE_BASE;
                y = height / CameraMaskView.SIZE_BASE;
                Log.i("刘朝:ROTATION_270->","x : " + x +"; y : " + y);

                break;
            default:
                x = width / CameraMaskView.SIZE_BASE;
                y = height / CameraMaskView.SIZE_BASE;
                Log.i("刘朝:default->","x : " + x +"; y : " + y);
                break;
        }
